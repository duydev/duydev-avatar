                        ...
                        @if( $frames->count() > 0 )
                            @foreach( $frames as $frame )
                                <div class="col-md-3 item">
                                    ...
                                    <button class="btn btn-danger" onclick="if( confirm('Bạn thực sự muốn xóa khung {{ $frame->title }}?') ){ event.preventDefault(); document.getElementById('delete-form-{{ $frame->id }}').submit(); }" ><i class="fa fa-trash"></i> Xóa khung</button>
                                    {{ Form::open(['method'=>'delete','route'=>['delete_frame',$frame->slug],'class'=>'hidden','id'=>"delete-form-$frame->id"]) }}
                                    {{ Form::close() }}
                                </div>
                            @endforeach
                        @else
